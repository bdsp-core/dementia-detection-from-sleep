...
def compute_features_each_seg(eeg_seg, seg_size, channel_num, band_num, NW, Fs, freq, band_freq, total_freq_range, total_freq_id, window_length, window_step):#, dpss=None, eigvals=None):
    ...
    f1 = np.abs(np.diff(eeg_seg,axis=1)).sum(axis=1)*1.0/seg_size  # mean gradient
    ...
    return np.r_[f1,f2,f4,f5,f6,f7,f9,f10,f11,f12]#,f3,f8
...
def extract_features(EEG_segs, channel_names, combined_channel_names, Fs, NW, total_freq_range, sub_window_time, sub_window_step, seg_start_ids, return_feature_names=False, n_jobs=-1, verbose=True):
    ...
    features = Parallel(n_jobs=n_jobs,verbose=verbose)(delayed(compute_features_each_seg)(EEG_segs[segi], window_size, channel_num, band_num, NW, Fs, freq, band_freq, total_freq_range, total_freq_id, sub_window_size, sub_step_size) for segi in range(seg_num))#, dpss=dpss, eigvals=eigvals
