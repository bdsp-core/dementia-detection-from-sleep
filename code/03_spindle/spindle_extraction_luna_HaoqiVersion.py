...
# # generate aligned lists of signals and labels

# In[5]:


"""
signal_mat_paths = glob.glob('/media/mad3/Datasets_ConvertedData/sleeplab/grass_data/*/Signal_*.mat')+glob.glob('/media/mad3/Datasets_ConvertedData/sleeplab/natus_data/*/Signal_*.mat')
stage_paths = glob.glob('/media/mad3/Datasets_ConvertedData/sleeplab/grass_data/*/Labels_*.mat')+glob.glob('/media/mad3/Datasets_ConvertedData/sleeplab/natus_data/*/Labels_*.mat')

# In[6]:


signal_sids = [os.path.basename(x).replace('Signal_','') for x in signal_mat_paths]
label_sids = [os.path.basename(x).replace('Labels_','') for x in stage_paths]
common_sids = sorted(set(signal_sids) & set(label_sids))

signal_ids = [signal_sids.index(x) for x in common_sids]
label_ids = [label_sids.index(x) for x in common_sids]

signal_mat_paths_aligned = [signal_mat_paths[x] for x in signal_ids]
stage_paths_aligned = [stage_paths[x] for x in label_ids]
assert len(signal_mat_paths_aligned)==len(stage_paths_aligned)
"""

df = pd.read_excel('/data/brain_age/brain_age_AllMGH/mycode/subject_files.xlsx')
signal_mat_paths_aligned = df.signal_path.values
stage_paths_aligned = df.label_path.values
"""
signal_mat_paths_aligned = ['/media/mad3/Datasets_ConvertedData/sleeplab/natus_data/Thilbeault~ De_2589f639-c0ff-4dc4-ba34-a533385155e7/Thilbeault~ De_2589f639-c0ff-4dc4-ba34-a533385155e7.edf']
stage_paths_aligned = ['/media/mad3/Datasets_ConvertedData/sleeplab/natus_data/Thilbeault~ De_2589f639-c0ff-4dc4-ba34-a533385155e7/annotations.csv']
"""
...
output_dir = '/data/dementia_classification_ElissaYe/spindle_output-part2'  # must be an absolute path
xml_edf_dir = '/data/dementia_classification_ElissaYe/edf_xml_part2'
error_files_path = '/data/dementia_classification_ElissaYe/spindle_output-part2/error_files2.txt'
symlink_path = '/data/dementia_classification_ElissaYe/bad_files_name_replaced_by_symlink'

channels = ['C3M2', 'C4M1', 'F3M2', 'F4M1', 'O1M2', 'O2M1']

# generate edf and xmls files and paths
edf_paths = []
xml_paths = []
err_msgs = []
for signal_path, stage_path in tqdm(zip(signal_mat_paths_aligned, stage_paths_aligned), total=len(signal_mat_paths_aligned)):
...
# write error files
with open(error_files_path, 'w') as ff:
    ff.write('\n'.join(err_msgs))
...
# load existing luna subject ids
existing_batch_files = glob.glob(os.path.join(output_dir, 'luna_output_NREM2_batch*.xlsx'))
existing_sids = set()
existing_batch_ids = []
for bf in tqdm(existing_batch_files):
    df = pd.read_excel(bf)
    existing_sids.update(df.ID)
    existing_batch_ids.append(int(bf.split('_')[-1][5:-5]))
if len(existing_batch_ids)>0:
    base_batch_id = max(existing_batch_ids)+1
else:
    base_batch_id = 0

existing_mask = []
for ep in edf_paths:
    ep2 = os.path.basename(ep).replace('.edf','').lower()
    if 'twindata' in ep2:
        ep2 = ep2.replace(',','').replace('.','')
    existing_mask.append(ep2 in existing_sids)
edf_paths = np.array(edf_paths)[~np.array(existing_mask)]
xml_paths = np.array(xml_paths)[~np.array(existing_mask)]
...
n_batch = 100
batches = np.array_split(np.arange(len(edf_paths)), len(edf_paths))#n_batch)
dfs = []
i_batch = 0
