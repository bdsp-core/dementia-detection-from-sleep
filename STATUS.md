...
| `s3://.../sleep-dementia-detection/dementia_diagnosis_dates.csv` | ✅ | Renamed and column-cleaned from `MGH_dementia_all_04082025_Elissa_rule_made_by_Haoqi.csv` (orig source: `box:.../BDSP_deID/I0001-MGB/data_Outcomes/mild_cognitive_impairment/old/`). 23,828 rows; columns `BDSPPatientID, DiagnosisDateShifted, AgeAtDiagnosis`. **DiagnosisDateShifted is verified to use the same per-patient `ShiftedDays` offset** as `DOVshifted`/`DOBshifted` (validated by checking `Age == (DiagDate − DOB)/365.25 − ShiftedDays/365.25` to 1e-15 precision). The diagnosis date is the chart-review event date, not a PSG date. |
| `s3://.../sleep-dementia-detection/psg_manifest.csv` | ✅ | 10,782 rows × 9 cols. One row per analytic-cohort PSG. Columns: `BDSPPatientID, HashID, FileNameNew, DOVshifted, Sex, AgeAtPSG, PSGType, group, s3_path`. Lets users fetch raw PSGs from `s3://bdsp-opendata-credentialed/I0001-MGB/<FileNameNew>` and re-run feature extraction with `code/02_features/main_BA.py` + `main_spindle_SO.py`. |
