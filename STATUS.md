...
This repository assembles the project's complete working tree from internal collaboration folders into a single public layout — code (~70 notebooks), feature tables, regex/criteria files, and the published-figure SVGs. PHI source data is staged locally for the de-identification pipeline; only deidentified outputs land on S3.
...
| File | Status | Source |
|---|---|---|
| `step2b/c_*ElissaCriteria*.py` | 🟡 | the internal labeling pipeline — hard-coded paths to `medical_data/`. Adapt path or set up symlink to `_box_inspect/dementia_detection_other/medical_data/`. |
| `Dementia_criteria_Elissa_step{1,2}.ipynb` | 🟡 | dropbox `sleep_stressor/data/sleep_general/examples/` |
| `assign_study_groups_V3.ipynb` | ✅ | (internal project folder) — produces the DEM/MCI/CN study-group assignment from the labeled cohort. |
| `wrangle_neuropsychiatric_scores.ipynb`, `mine_neuropsychiatric_scores_V2.ipynb` | ✅ | same |
| `ICD_Code_Analysis.ipynb` | ✅ | same |
| `get_*.ipynb` (5 files) | ✅ | (internal project folder) — the 2018 RPDR extraction notebooks |
| `exclusion_regex`, `dementia_regex`, `MCI_regex.txt`, `medications_regex.txt` | ✅ | (internal project folder) |
...
| File | Status | Source |
|---|---|---|
| `segment_EEG.py`, `multitaper_spectrogram.py`, `bandpower.py`, `extract_features_parallel.py`, `load_mgh_sleep_dataset.py` | ✅ | (internal Dropbox folder) |
| `main_BA.py`, `main_spindle_SO.py` | ✅ | same |
| `compile_brain_age_features.ipynb`, `combine_luna_output.ipynb` | ✅ | (internal project folder) — drivers that assemble the final per-PSG feature table |
...
| File | Status | Source |
|---|---|---|
| `LUNAspindles/` | ✅ | (internal project folder) — Python wrapper around Luna |
| `spindle_extraction_luna_HaoqiVersion.py` | ✅ | (internal project folder) — the spindle extraction wrapper used for the SLEEP 2023 paper |
| `summary.py`, `destrat.py`, `fit_run.py`, `fit_boot.py`, `id_pairs.py`, `luna_07*.py`, `spindleCommand.txt` | ✅ | (internal project folder) |
...
| File | Status | Source |
|---|---|---|
| **`binary_dementia_classification_DM_v_CN_V2.ipynb`** | ✅ | (internal project folder) — **canonical paper notebook for DEM vs CN** (resolves Tier-1 #1). Uses LR + SVM + RF + Lasso feature selection + Stratified KFold + AUROC/AUPRC. |
...
| File | Status | Source |
|---|---|---|
| `plot_confusion_matrix.ipynb` | ✅ | (internal project folder) — paper Fig. 4 |
| `sleepdata_summary_statistics_V3.ipynb` | ✅ | same — paper Table 1 |
| `plot_*.py`, `generate_*.py` (SBOP figure scripts) | 🟡 | internal figure scripts — for the SBOP survival paper, kept as templates for figure re-use |

The paper's published Figs. 1A/1B/4A/4B/4I are also available in raw `.svg` form in (internal project folder).
...
| File | Status | Source |
|---|---|---|
| `data/features_MGH_deid.csv` | ✅ | (internal Dropbox folder) — 8,673 × 158 deidentified per-PSG feature matrix from the SBOP cohort. **Subset of the SLEEP 2023 cohort** (excludes prevalent DEM/MCI cases). |
| `data/mastersheet_outcome_deid.xlsx` | ✅ | same — `HashID ↔ BDSPPatientID` crosswalk + per-PSG survival outcomes. |
| `s3://.../sleep-dementia-detection/dementia_diagnosis_dates.csv` | ✅ | Renamed and column-cleaned from `MGH_dementia_all_04082025_Elissa_rule_made_by_Haoqi.csv` (orig source: (internal project folder)). 23,828 rows; columns `BDSPPatientID, DiagnosisDateShifted, AgeAtDiagnosis`. **`DiagnosisDateShifted` is the date of the clinical encounter where dementia was documented** (not the chart-review date — review happened in 2025, but the dates here range 1999-2024). Verified shifted by the same per-patient `ShiftedDays` offset as `DOVshifted`/`DOBshifted` (validated by checking `Age == (DiagDate − DOB)/365.25 − ShiftedDays/365.25` to 1e-15 precision). |
...
⚠ Empty in the repo. Trained pickles exist in (internal project folder):
